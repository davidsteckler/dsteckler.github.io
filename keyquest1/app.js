(() => {
  ...
  function cleanSentence(text){
    return text.replace(/\s+/g,' ').replace(/\.\./g,'.').replace(/([.!?])\./g,'$1').trim();
  }
  ...
  function countChar(text,char){
    ...
    return [...text.toLowerCase()].reduce((n,x)=>n+(x===c?1:0),0);
  }

  function recentFromStorage(){
    try{return JSON.parse(localStorage.getItem('keyquest1-recent-concepts')||'[]').slice(-80)}
    catch{return []}
  }
  function rememberConcept(name){
    const saved=recentFromStorage().filter(x=>x!==name);
    saved.push(name);
    localStorage.setItem('keyquest1-recent-concepts',JSON.stringify(saved.slice(-80)));
  }
  ...
  function chooseConcept(options={}){
    ...
    if(options.char){
      const ranked=candidates.map(i=>({
        i,
        hits:countChar(knowledge[i].slice(1).join(' '),options.char)
  ...
  function makeSingle(item, frameIndex=null){
    const fi=frameIndex===null?Math.floor(Math.random()*frames.length):frameIndex%frames.length;
    return {text:cleanSentence(frames[fi](item)), frame:fi};
  }
  ...
  function learningSentence(){
    ...
    conceptChip.textContent=item[1];
    ...
    if(usePair){
      ...
      const pairForm=Math.floor(Math.random()*forms.length);
      text=cleanSentence(forms[pairForm]);
      signature=`p-${index}-${otherIndex}-${pairForm}`;
    }else{
      let made;
      let tries=0;
      do{
        made=makeSingle(item);
        signature=`s-${index}-${made.frame}`;
        tries++;
      }while(state.recentSignatures.includes(signature)&&tries<12);
      text=made.text;
    }

    state.recentSignatures.push(signature);
    if(state.recentSignatures.length>70)state.recentSignatures.shift();
    return text;
  }
  ...
  function speedSentence(){
    ...
    $('topicLabel').textContent='Flow practice';
    const count=3+Math.floor(Math.random()*2);
    const selected=[];
    const used=new Set();
    for(let i=0;i<count;i++){
      let choice=chooseConcept();
      let guard=0;
    ...
    const flowFrames=[
      ...
      k=>`${k[4]}.`,
      k=>`${k[2]}. ${k[4]}.`,
      k=>`${k[1]} is useful to know because ${lower(k[3])}.`,
      k=>`A practical computing example is this: ${lowerFirst(k[4])}.`,
      k=>`One computer science fact worth remembering is that ${lower(k[2])}.`
    ];
    return selected.map(k=>cleanSentence(pick(flowFrames)(k))).join(' ');
  }

  function mostMissedKey(){
  ...
  function precisionSentence(){
    state.currentKnowledge=null;
    conceptChip.hidden=true;
    const key=mostMissedKey();
    state.precisionKey=key;

    if(key && /[0-9=+>:\[\]()"'.,;<>/%{}_-]/.test(key) && Math.random()<0.7){
      $('topicLabel').textContent='Programming symbols';
      precisionHint.textContent=`Targeting ${key===' '?'space':key} through real code patterns.`;
      const matching=precisionCode.filter(s=>s.includes(key));
      return pick(matching.length?matching:precisionCode);
    }

    const chosen=chooseConcept(key?{char:key}:{});
    if(!chosen)return pick(precisionCode);
    const item=chosen.item;
    $('topicLabel').textContent=key?`Focus key: ${key.toUpperCase()}`:'Accuracy + control';
    precisionHint.textContent=key
      ? `Adapting toward ${key.toUpperCase()} because it is showing up in your mistakes.`
      : 'Accuracy first. The target will adapt after it sees your mistakes.';

    const shortForms=[
      `${item[1]}: ${item[2]}.`,
      `${item[4]}.`,
      `${item[1]} matters because ${lower(item[3])}.`,
      `${item[2]}.`
    ];
    return cleanSentence(pick(shortForms));
  }
  ...
  function handleKey(e){
    ...
    if(e.key===expected){
      ...
      setTimeout(()=>{
        if(state.errorIndex===state.position){
          state.errorIndex=-1;
          updateChars();
        }
      },220);
  ...
  function updateLiveStats(){
    ...
    if(state.mode==='speed'){
      const current=wpm();
      const best=Math.max(getBest(),1);
      const pct=Math.min(100,Math.max(3,current/(Math.max(best,30)*1.2)*100));
      $('paceDot').style.left=`${pct}%`;
      $('paceText').textContent=`${current} wpm`;
  ...
  function tick(){
    ...
    const now=Date.now();
    const remaining=(state.endAt-now)/1000;
  ...
  function startSession(mode){
    ...
    state.endAt=state.startedAt+state.minutes*60*1000;
    state.lastKeyAt=0;
    state.activeMs=0;
    state.correct=0;
    state.attempts=0;
    ...
    conceptChip.hidden=mode!=='learn';
    $('sentenceCount').textContent='0 sentences';
    $('wpmValue').textContent='0';
    $('accuracyValue').textContent='100';
    $('timeValue').textContent=formatTime(state.minutes*60);
  ...
  function endSession(){
    ...
    $('resultWpm').textContent=String(finalWpm);
    $('resultAccuracy').textContent=`${accuracy()}%`;
    $('resultSentences').textContent=String(state.completed);
    $('resultActive').textContent=formatTime(state.activeMs/1000);
    $('resultBest').textContent=newBest?`${newBest} WPM`:'—';
    $('resultTitle').textContent=finalWpm>oldBest&&finalWpm>0?'New personal best.':'Nice run.';

    const learned=$('resultLearned');
    const chips=$('learnedChips');
    chips.innerHTML='';
    if(state.mode==='learn'&&state.seenConcepts.length){
      learned.hidden=false;
      state.seenConcepts.slice(-12).forEach(c=>{
        const span=document.createElement('span');
        span.textContent=c;
  ...
  document.querySelectorAll('[data-minutes]').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    document.querySelectorAll('[data-minutes]').forEach(b=>b.classList.toggle('selected',b===btn));
  }));
  $('endButton').addEventListener('click',endSession);
  homeButton.addEventListener('click',showHome);
  $('modesButton').addEventListener('click',showHome);
  $('againButton').addEventListener('click',()=>startSession(state.mode));
